...
public class FloorMap : MonoBehaviour
{
    ...
    void GenerateDungeon(int seed, int level)
    {
        ...
        //      - then, speedrunners/better players have negative feedback loop (?) [do we even want that?]
        // - positive feedback loop with perma cosmetic :D or shop creds

        // IMPORTANT: THIS IS NOT A ROGUELIKE
        // shop items should be temporary one-time usage or perma cosmetic
        // - if ppl replay the game, i think dialogue should be different the second time
